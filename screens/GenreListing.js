...
import { Filigree1, Filigree4 } from '../components/decorations/Filigree';
import { useDispatch, useSelector } from 'react-redux';
import ScreenTitle from '../components/ScreenTitle';
import { fetchGenre, setBookListingTitle, setSearchKeyword, searchForBooks } from '../store/slices/bookSlice';


const GenreDisplay = ({ genreList }) => {
    const data = genreList;
    const navigation = useNavigation();
    const bookDatabase = useSelector((state) => state.books.booksDatabase);
...
const GenreItem = ({ navigation, genre, bookDatabase }) => {
    ...
    return (
        <TouchableOpacity style={styles.gi_container}
            onPress={() => {
                dispatch(setBookListingTitle("Thể loại"))
                dispatch(setSearchKeyword(genre.name))
                dispatch(searchForBooks(genre.name))
...
const GenreListing = ({ }) => {
    const dispatch = useDispatch();
    const { loading, genreDatabase } = useSelector((state) => (state.books));
    useEffect(() => {
        dispatch(fetchGenre());
    }, [])
    ...
    return (
        <View style={styles.container}>
            ...
            <ScrollView bounces={false} overScrollMode="never">
                ...
                <GenreDisplay genreList={genreDatabase} />
